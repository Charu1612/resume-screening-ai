...
class SkillMatcher:
    ...
    def analyze_match(self, resume_data: Dict, job_data: Dict) -> Dict:
        ...
        try:
            ...
            score_breakdown = self._calculate_score_breakdown(resume_data, job_data, skill_analysis)
    ...
    def _calculate_score_breakdown(self, resume_data: Dict, job_data: Dict, skill_analysis: Dict) -> Dict:
        """Calculate detailed score breakdown for frontend display"""
        # Skill matching score
        total_job_skills = len(skill_analysis['matched_skills']) + len(skill_analysis['missing_skills'])
        skill_match = (len(skill_analysis['matched_skills']) / total_job_skills * 100) if total_job_skills > 0 else 0
        
        # Skill frequency score (simplified - based on number of matched skills)
        skill_frequency = min(len(skill_analysis['matched_skills']) * 10, 100)
        
        # Experience match score
        experience_match = self._calculate_experience_score(resume_data, job_data) * 100
        
        # Education match score
        education_match = self._calculate_education_score(resume_data, job_data) * 100
        
        # Keywords/tools score
        keywords_tools = self._calculate_keyword_score(resume_data, job_data) * 100
        
        return {
            'skill_match': round(skill_match, 1),
            'skill_frequency': round(skill_frequency, 1),
            'experience_match': round(experience_match, 1),
            'education_match': round(education_match, 1),
            'keywords_tools': round(keywords_tools, 1)
        }
