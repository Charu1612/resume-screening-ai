...
@resume_bp.route('/analyze-direct', methods=['POST'])
@jwt_required()
def analyze_resume_direct():
    try:
        ...
        # Save temporary file
        filename = secure_filename(f"temp_{user_id}_{file.filename}")
        ...
        try:
            ...
            # Create job data from description
            job_data = {
                'description': job_description,
                'title': 'Custom Job Analysis',
                'requiredSkills': []  # Will be extracted by the matcher
            }
            
            # Perform analysis
            matcher = SkillMatcher()
            analysis = matcher.analyze_match(resume_data, job_data)
