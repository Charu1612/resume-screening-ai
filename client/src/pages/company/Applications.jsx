...
const Applications = () => {
  ...
  const fetchApplications = async () => {
    try {
      ...
      console.log('Mapped applications:', applicationsData)
      setApplications(applicationsData)
    } catch (error) {
      console.error('Error fetching applications:', error)
      console.error('Error response:', error.response?.data)
      // For demo purposes, show a message instead of mock data
      setApplications([])
      alert(`Failed to load applications: ${error.response?.data?.error || error.message}. Please make sure you are logged in as a company.`)
    } finally {
      setIsLoading(false)
    }
  }
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                {selectedApplication.resumeUrl ? (
                  <a
                    href={`http://localhost:5000/uploads/${selectedApplication.resumeUrl.split('/').pop()}`}
                    target="_blank"
